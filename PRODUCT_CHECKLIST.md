...
- 2026-04-06 current pass: council captions and floor ownership chrome were separated vertically, and featurette/documentary action buttons were retuned toward the nicer cinematic pill treatment used in the main rooms.
- 2026-04-06 current pass: voice citizen targeting now accepts plain-language descriptors through the existing focus tool path, including “nearest person,” “a kid,” “a college student,” “a small business owner,” and similar requests when the roster has a plausible match.
- 2026-04-06 current pass: debate and council prompts were tightened again against prepared-list behavior and soft pro-regulation mirroring. The opponent flagship move now prioritizes a pro-capability / narrower-remedy contrast when the player takes a broad-brake, high-tax, licensing, cap, or pause stance.
- 2026-04-06 current pass: featurette and town-hall prompts were tightened again against comma-separated inventories, staff-written voter questions, and generic framework/safeguard phrasing.
- 2026-04-06 lunch pass: headed Chrome-for-Testing audio probes rechecked the town-hall, council, and solo-advisor voice paths on `sim_b2cc9f5eaf82`. Solo realtime answered with no voice error, council hybrid captured the fake mic and played a named advisor response, and town hall captured the player answer then generated/played the opponent reply through the streaming local audio path.
- 2026-04-06 lunch pass: town hall local TTS now marks itself as external playback for the hybrid listener. This suppresses the voter/opponent audio echo from being mistaken for player speech while keeping the normal single-advisor and street realtime paths untouched.
- 2026-04-06 lunch pass: side documentary reels now have a playback stall guard and their overlay `Back to reels` action clears a requested featurette id instead of reopening the same reel immediately. Reels remain full-screen/cinematic in the checked Mexico run.
- 2026-04-06 lunch pass: public mood boards now prefer stable poll slots, avoid blank generic metrics, and clean up quote-like poll snippets before rendering them. Policy boards now render shorter three-item display labels so stale long policy notes do not spill across the back-wall board.
- 2026-04-06 lunch pass: optional featurette resume on backend startup is capped to the newest stale jobs, so old queued featurettes can be unstuck without rehydrating a large pile of historical media jobs and slowing the machine.
- 2026-04-06 lunch pass: the stage-cycle screenshot harness was hardened with forced cinematic-control clicks and an outer screenshot timeout, but repeated headed screenshot capture can still wedge Chrome-for-Testing's GPU path under heavy local load. Earlier stage-cycle screenshots and the newer audio/reel checks remain the more reliable evidence set for this pass; stale browser processes were killed after the failed capture.
- 2026-04-06 lunch pass: validation after the code changes is clean: `npm --prefix web run build`, `python -m pytest -q tests/test_prompt_quality.py tests/test_director.py tests/test_gabriel_service.py`, JS syntax checks for the Playwright harnesses, and `git diff --check` all passed.

- 2026-03-25 current pass: a fresh initializing run (`sim_09804086c9e0`) now shows an orchestrator-generated council roster in state (`Tanya`, `Arjun`, `Elena`, `Marcus`, `Dana`) instead of silently falling back to the stock Rowan/Leila/Mateo/Amina quartet. This confirms the backend roster generation path is now live on new runs.
- 2026-03-25 current pass: the live room transcript no longer duplicates freshly sent user/advisor turns just because optimistic local turns and persisted backend turns carry different ids. Frontend merging now coalesces adjacent near-identical turns by content/time instead of id only.
- 2026-03-25 current pass: visible Chrome-for-Testing QA on `sim_cd0e2ca3b5f4` revalidated the solo advisor, multi-advisor, and town-hall rooms after the latest council changes. Text turns now land cleanly again in solo/council, and town hall successfully opens into an audience-question flow instead of hanging on an inert button state.
- 2026-03-25 current pass: fresh fullscreen scene QA on `sim_40072aee1b84` shows the main in-world mic no longer flips straight back to idle. In browser smoke, the advisor-room scene button now moves from `Speak` to a live `Pause` state and stays there instead of self-resetting immediately after click.
- 2026-03-25 current pass: the council loop no longer double-persists every user/advisor beat through both `generate_council_turn` and a second frontend sync. The council endpoint is now treated as the authoritative persistence path, which removes an avoidable request from every council beat and should trim some of the multi-advisor latency.
- 2026-03-25 current pass: council turn fetches are now abortable, and council prefetches are explicitly cancelled on interrupt / pause / disconnect. That should reduce stale follow-up replies leaking through after the player cuts in or changes rooms.
- 2026-03-25 current pass: broad national runs are less likely to get silently narrowed into a sector electorate just because the player mentions a topic like education or health. Explicit region scope can still narrow the sample, but broad runs now stay broad and weighted instead of being auto-replaced.
- 2026-03-25 current pass: phase progression logic was simplified from three hard-coded `default/advanced/radical` ladders into one base arc plus later-start offsets. The setup is still guided by natural-language future cues, but the internal progression is less canned than before.
- 2026-03-25 current pass: local headless Playwright still cannot render the Three.js scene faithfully because Chromium-in-headless cannot create a WebGL context here. DOM-level fullscreen QA is still useful, but visual scene validation needs a real browser session rather than relying on headless screenshots.
- 2026-03-25 current pass: the setup chamber mic was rechecked against the integrated `:8000` app path after a clean rebuild/restart. `Speak -> Joining -> Stop/Pause -> Speak` now holds in browser QA instead of dropping straight back to idle, which was the main fullscreen setup-room regression from the latest report.
- 2026-03-25 current pass: the local Vite path is healthy again after the clean server restart. `http://127.0.0.1:5173` no longer throws the earlier localhost CORS failures during setup boot.
- 2026-03-25 current pass: backend council latency was trimmed again. The council draft and decider passes now run at reasoning `none`, which should cut some avoidable delay from multi-advisor turns without changing the draft-plus-decider architecture.
- 2026-03-25 current pass: advisor-council prompt hints are now roster-derived instead of hard-coded to Rowan/Leila/Mateo/Amina. The shared roster block helper also now respects the live simulation roster everywhere, which closes one more dynamic-council drift path.
- 2026-03-25 current pass: council turn routing is now aligned to the backend draft-plus-decider contract. The frontend no longer tries to second-guess floor control with extra urgency heuristics; it follows `next_speaker`, continues only while the room still owns the floor, and keeps the repeat-suppression safety.
- 2026-03-25 current pass: the immersive mic no longer tears down live advisor/citizen/debate channels when the player taps it again. Scene voice now hot-pauses and resumes through the same session, matching the setup chamber more closely and reducing reconnect churn.
- 2026-03-25 current pass: dynamic council rosters are now threaded through the frontend. Council parsing, council speech synthesis, the dock transcript, and the in-room 3D advisor labels all read from the simulation roster instead of a hard-coded four-name table.
- 2026-03-25 current pass: setup and in-scene mic controls now present pause/resume language when a live channel is muted instead of continuing to imply a full disconnect. Street-selection locking also releases correctly while the mic is paused.
- 2026-03-25 current pass: local dev voice QA is no longer brittle to a bumped Vite port. The backend now accepts localhost and 127.0.0.1 origins across arbitrary local ports, which keeps setup/advisor mic testing from failing on pure CORS noise when the frontend lands on 5174 or 5175.
